#!/usr/bin/env python3
"""Non-interactive client for the MAF-native MicroHarness HTTP endpoint."""
...
DEFAULT_PROMPT = (
    "Explica la variante MAF-native del harness. "
    "Separa AgentSession, ContextProvider, FileHistoryProvider, "
    "MemoryContextProvider, TodoProvider y middleware."
)


def main() -> int:
    """Run one request against the local MAF-native server endpoint."""
    ...
    try:
        response = httpx.post(
            f"{base_url.rstrip('/')}/api/chat/maf-native",
            json={
                "prompt": prompt,
                "session_id": "cli-maf-native",
                "allow_reference_response": True,
            },
            timeout=90.0,
        )
    ...
    print("=== Respuesta del agente MAF-native ===")
    print(answer)
    print()
